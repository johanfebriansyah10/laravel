<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl">Tambah Dosen</h2>
    </x-slot>

    <form method="POST" action="{{ route('admin.register.dosen.store') }}" class="p-6 space-y-4">
        @csrf
        <input name="name" placeholder="Nama" required class="w-full p-2 border rounded" />
        <input name="nidn" placeholder="NIDN" required class="w-full p-2 border rounded" />
        <input name="gelar" placeholder="Gelar" class="w-full p-2 border rounded" />
        ...
        <input name="alamat" placeholder="Alamat" class="w-full p-2 border rounded" />
